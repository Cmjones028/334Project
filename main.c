...
void moveEnemies(bool enemies[NUM_ROWS][NUM_COLS]) {

    bool tempEnemies[NUM_ROWS][NUM_COLS] = {false};

    // Move each enemy one space to the left
    for (int r = 0; r < NUM_ROWS; r++) {
        ...
        for (int c = 1; c < NUM_COLS; c++) {
            ...
            if (enemies[r][c]) {
                ...
                tempEnemies[r][c - 1] = true;
            }
        }
    }

    // Copy the temporary grid back into enemies
    for (int r = 0; r < NUM_ROWS; r++) {

        for (int c = 0; c < NUM_COLS; c++) {

            enemies[r][c] = tempEnemies[r][c];
        }
    }
}
...
int main(void) {
    ...
    while (1) {
        ...
        //places the enemies on the real board
        for (int r = 0; r < NUM_ROWS; r++) {
            for (int c = 0; c < NUM_COLS; c++) {
                if (enemies[r][c]) {
                    screen.cells[r][c] = 'X';
